...
def get_config(config_string="full,multimodal"):
    ...
    action_horizon = FieldReference(default=4)
    
    dataset_kwargs = dict(
        oxe_kwargs=dict(
                data_mix="robocasa_atomic_static_tasks",
        ...
        traj_transform_threads=24, # MUST BE SPECIFIED FOR PyTorch training !!!!
        traj_read_threads=24, # MUST BE SPECIFIED FOR PyTorch training !!!!
    )
    ...
    config = dict(
        ...
        seed=42,
        wandb=dict(
            project="octo_finetune_pt", group=placeholder(str), entity=placeholder(str)
        ),
